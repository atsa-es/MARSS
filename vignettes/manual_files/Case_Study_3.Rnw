...
\index{MARSS model!multivariate example}In this application, we use harbor seal abundance estimates along the west coast to examine large-scale spatial structure.  Harbor seals are distributed along the west coast of the U.S. from California to Washington.  The populations been surveyed at haul-out sites since the mid-1970s (Figure \ref{fig:CS3.fig1}) and have been increasing steadily since the 1972 Marine Mammal Protection Act. See \verb@?harborSeal@ for the data sources.
