...
The \verb@cars@ dataset in R contains 50 data points on stopping distances (ft) and speed (mph) for a car in 1920.  We can describe this dataset with a linear relationship between car speed ($y$) and stopping distance ($x$): $y = \alpha + \beta x + e$, where $e$ are errors with some distribution.  Let's assume (for the moment) that those errors have a Gaussian distribution, $e \sim N(0, \sigma^2)$.  If we fit this model to the data, we get the red line in Figure \ref{fig:CIs.basics}a. We will call this fit \verb@fit@:
<<Cs_001_fit-full>>=
fit=lm(dist~speed, data=cars)
fit
@

This fit has $\alpha$=\Sexpr{round(coef(fit)[1],digits=2)} and $\beta$=\Sexpr{round(coef(fit)[2],digits=2)}. We are going to treat this relationship (the red line) as the 'truth' that we are trying to estimate.  We will estimate this true red line using a smaller sample (10 cars) from the original 50-car data set.

Let's imagine one of these 10-car samples, shown as crosses in Figure \ref{fig:CIs.basics}a.  From this new data set, we can calculate estimates of $\alpha$ and $\beta$ ($\hat{\alpha}$ and $\hat{\beta}$) and produce the black line in Figure \ref{fig:CIs.basics}a.  $\hat{\alpha}$ and $\hat{\beta}$ would not match $\alpha$ and $\beta$ since we used a sample of 10 cars from the original 50 cars.  We could repeat this for many 10-car samples and there would be many possible regression lines (black lines in Figure \ref{fig:CIs.basics}b).  Here is  code to create a large number of regression lines using 10-car samples:
<<Cs_001_fit-i>>=
nsim = 5000
...
Let's consider one particular sample of 10 cars which we will call the $j$-th sample. There are many 10 car samples and this is just one of them.  We will define this sample as the cars in rows $9, 11, 12, 13, 19, 20, 21, 22, 23, 24$ in the \verb@cars@ dataset:
<<Cs_001_fit-j>>=
smpj = c(9, 11, 12, 13, 19, 20, 21, 22, 23, 24)
cars.j = cars[smpj,]
fit.j = lm(dist~speed, data=cars.j)
@
Figure \ref{fig:CIs.basics}c shows the regression line for this sample. This line is our estimate of the expected value of new stopping data for any speed $x$. We will call this expected value $E[\tilde{y}|x]$ where the $|$ means 'given'.

Given data set $j$, that estimate is $\widehat{E[\tilde{y}|x]}=\hat{\alpha}_j + \hat{\beta}_j x$.

The expected value of new stopping data generated with the fitted model $\tilde{y}=\hat{\alpha}_j + \hat{\beta}_j x + e$ is $\hat{\alpha}_j + \hat{\beta}_j x$.    We can construct a confidence interval for $E[\tilde{y}|x]$. Let's call this interval: $CI_j|x$;   This 95\% confidence interval is an interval constructed in such a way that for 95\% of the possible 10-car samples, the confidence interval will include (or cover) the true value of $E[\tilde{y}|x]$ which is $\alpha + \beta x$.

Before proceeding, we should clarify how the variability in $\tilde{y}$ arises---what is the nature of the data we did not collect but that might have collected or is out there to be collected.  One possibility is that we decided to only measure mpg in cars with weights $\mathbf{x}$ (meaning the set of weights in the mtcars dataset.  In this scenario, the weights were a choice on our part.  We imagine that there are other car models out there with the same weights but different mpgs and that new 32-car datasets would follow this relationship:
$$\tilde{\yy}_i = \alpha + \beta \xx + \ee_i$$
where $\tilde{\yy}_i$ are the 32 mpgs for the cars in the $i$-th 32-car sample, $\xx$ are the 32 weights of the cars and are the same weights in the mtcars sample, and $\ee_i$ are the 10 residual errors.  The weights are the same as in the $j$-th sample but the mpgs are different because $\ee_i$ is different than $\ee_j$.  
...
plot(mtcars$wt, mtcars$mpg, ylim=c(5,30), ylab="", xlab="")
abline(fit, col="red", lwd=2)

#regression line from the j-th sample of 10 cars
set.seed(222)
mtcars.j = mtcars
mtcars.j$mpg = alpha + beta*wt + rnorm(32,0,sigma)
...
#distribution of the regression lines from 32-car samples
#j sample of 10 cars
nsim = 5000
predx=seq(2,5,0.5); nx = length(predx)
i.results=matrix(NA,nsim,3)
i.ci95 = matrix(NA,nsim,3*nx)
i.ci75 = matrix(NA,nsim,3*nx)
for(i in 1:nsim){
  mtcars.i = mtcars
  mtcars.i$mpg = alpha + beta*wt + rnorm(32,0,sigma)
  fit.i = lm(mpg~wt, data=mtcars.i)
  i.results[i,]=c(coef(fit.i), sum(fit.i$residual^2)/fit.i$df.residual)
  i.ci95[i,] = as.vector(predict(fit.i, newdata=data.frame(wt=predx), interval="confidence"))
  i.ci75[i,] = as.vector(predict(fit.i, newdata=data.frame(wt=predx), interval="confidence", level=0.75))
}
#plot out 1000 of the regression lines
plot(mtcars$wt,mtcars$mpg, ylim=c(5,30), type="n", ylab="", xlab="")  
title("1000 bootstrapped regression lines\nfrom 10 car samples",cex.main=.75)
for(i in 1:1000){ lines(c(1,6), i.results[i,1]+i.results[i,2]*c(1,6)) }
legend("topright","(b)",bty="n")
abline(fit, col="red", lwd=2)

#confidence intervals
plot(mtcars$wt, mtcars$mpg, ylim=c(0,30), type="n", ylab="mpg", xlab="weight")
abline(fit, col="red", lwd=2)
points(mtcars.j$wt, mtcars.j$mpg, pch=3)
abline(fit.j)
...
#Coverage
true.y = predict(fit, newdata=data.frame(wt=predx))
ci.obs95 = ci.obs75 = rep(NA,nx)
for(i in 1:nx){
  ci.obs95[i] = sum(i.ci95[,i+nx]<true.y[i] & i.ci95[,i+2*nx]>true.y[i])/nsim
  ci.obs75[i] = sum(i.ci75[,i+nx]<true.y[i] & i.ci75[,i+2*nx]>true.y[i])/nsim
}
