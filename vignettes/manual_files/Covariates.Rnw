...
@
\end{center}
\caption{Time series of Green and Blue-green algae abundances in Lake Washington along with the temperature and total phosphorous covariates.}
...
@
The model selection results indicate that the model with monthly seasonal effects estimated via the discrete Fourier sequence is the most parsimonious of the 3 models. Its AICc value is much lower than either the polynomial or fixed-effects models.
...
for(i in 1:5) {
  plot.ts(MARSSresiduals(seas.mod.1)$model.residuals[i,], 
  ...
  acf(MARSSresiduals(seas.mod.1)$model.residuals[i,], lag.max=period, na.action=na.pass)
  if(i==5) {
...
par(mfrow=c(5,2), mai=c(0.1,0.5,0.2,0.1), omi=c(0.5,0,0,0))
for(i in 1:5) {
  plot.ts(MARSSresiduals(seas.mod.2)$model.residuals[i,], 
      ylab="Residual", main=phytos[i], xlab="", xaxt="n")
  abline(h=0, lty="dashed")
  if(i==5) {
       axis(1, at=1+seq(0,TT-period,by=12), 
  ...
  acf(MARSSresiduals(seas.mod.2)$model.residuals[i,], lag.max=period, na.action=na.pass)
  if(i==5) {
       axis(1, at=c(0,seq(period)))
       mtext(side=1, line=2.7, "Time lag")
     }
...
par(mfrow=c(5,2), mai=c(0.1,0.5,0.2,0.1), omi=c(0.5,0,0,0))
for(i in 1:5) {
  plot.ts(MARSSresiduals(seas.mod.3)$model.residuals[i,], 
      ylab="Residual", main=phytos[i], xlab="", xaxt="n")
  abline(h=0, lty="dashed")
  if(i==5) {
       axis(1, at=1+seq(0,TT-period,by=12), 
  ...
  acf(MARSSresiduals(seas.mod.3)$model.residuals[i,], lag.max=period, na.action=na.pass)
  if(i==5) {
       axis(1, at=c(0,seq(period)))
       mtext(side=1, line=2.7, "Time lag")
     }
...
@
\texttt{fit.aug} is the MLE object that can be passed to \verb@MARSSkf()@.  You need to make a version of this MLE object with the non-covariate data filled with NAs so that you can compute the logLik without the covariates.  This needs to be done in the \texttt{marss} element since that is what is used by MARSSkf().  Below is code to do this.
