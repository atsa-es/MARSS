...
For this chapter, we will use a data set on car miles per gallon (mpg) in the city versus the car weight (1000s of pounds).  The data set has information on 141 cars models from 2009 (Figure \ref{fig:dat}).
...
\begin{figure}[htp]
\begin{center}
<<Cs_000_fig0, fig=TRUE, echo=FALSE, include.source=FALSE, keep.source=TRUE, width=6, height=6>>=
n = dim(dat)[1]
fit = lm(city~wt, data=dat)
...
plot(dat$wt, dat$city, ylim=c(0,35), ylab="in-city mpg", xlab="weight (1000s lbs)")
abline(fit, col="red", lwd=2)
m=10000
wt=runif(m,2,6)
mpg=alpha+beta*wt+rnorm(m,0,sigma)
points(wt,mpg,pch=".",col="red")
points(dat$wt,dat$city,pch=1,col="blue")
legend("topright", c("2009 car data"),pch=1,col="blue",bty="n")
legend("top",expression(hat(beta) == (X^t * X)^{-1} * X^t * y))
title("
@
\end{center}
\caption{The car mpg versus weight data set}
\label{fig:dat}
\end{figure}
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
We will assume that these data are a random sample from this generating process:
\begin{equation}\label{eqn.mpg}
y = \alpha + \beta x + e, \quad\quad e \sim N(0,\sigma)
\end{equation}
That is, that car mpg is a linear function of car weight with Gaussian errors.  


We will assume (for the moment) that at first that $e$ has a Gaussian (Normal) distribution: $e \sim N(0, \sigma)$.  We will assume that $\alpha$ = \Sexpr{round(alpha,digits=3)}, $\beta$= \Sexpr{round(beta,digits=3)}, and $\sigma$=\Sexpr{round(sigma,digits=3)} based on data on 140 U.S. car models in 2009.

Let's consider one particular 32-car sample from Equation \ref{eqn.mpg}. We will call this the $j$-th sample. There are many possible 32-car samples and this is a particular one: the sample that we observe or collect.  The other samples are hypothetical---they are other 32-car samples that could be generated from Equation \ref{eqn.mpg}.

We will define the $j$-th sample by setting the random seed to 222. We will randomly generate both $y$ (mpgs) and $x$ (weights).
<<Cs_001_fit-j>>=
set.seed(222)
nsamp=32
wt = runif(32, 2.5, 5.5)
mpg= alpha + beta*wt + rnorm(32,0,sigma)
...
Figure \ref{fig:CIs.basics}c shows the regression line for this sample. Given our estimates, $\hat{\alpha}_j$ and $\hat{\beta}_j$, from sample $j$, we can estimate the expected value of $\tilde{y}$ given any $x$. This is $\hat{\alpha}_j + \hat{\beta}_j x$.  Similarly we can construct a confidence interval for the expected value of $\tilde{y}$ given $x$. Let's call this interval: $CI_j|x$; the $|$ means 'given'.  It is a confidence interval for $E[\tilde{y}|x]$ based on our $j$-th 32-car sample.  This 95\% confidence interval is an interval constructed in such a way that for 95\% of the possible 32-car samples, the confidence interval includes (or covers) the true expected value: $\alpha + \beta x$.

We could imagine a different 32-car sample, $\tilde{\mathbf{y}}$, generated from this same model: $\tilde{\mathbf{y}} = \alpha + \beta \mathbf{x} + \mathbf{e}, e \sim N(0, \sigma^2)$. $\mathbf{x}$ is a vector of the 32 car weights in the mtcars data set.  $y$ and $x$ are bolded to indicate that they are vectors (32 values). The new data are shown as crosses in Figure \ref{fig:CIs.basics}a.  From this new data set, we can estimate $\alpha$ and $\beta$ to produce the black line in Figure \ref{fig:CIs.basics}a.  The $\alpha$ and $\beta$ estimated from $\tilde{\mathbf{y}}$ would not match the original values since we used a new sample of 32 cars.  We could repeat this for many 32-car samples and there would be many possible regression lines (black lines in Figure \ref{fig:CIs.basics}b).  Here is  code to create a large number of $\alpha$, $\beta$ and $\sigma^2$ estimates from 32-car samples:
<<Cs_001_fit-i>>=
nsim = 5000
alpha = coef(fit)[1]
beta = coef(fit)[2]
...
for(i in 1:nsim){
  new.mpg = alpha + beta*wt + rnorm(32,0,sigma)
  fit.i=lm(new.mpg~wt)
...
plot(dat$wt, dat$city, ylim=c(0,35), ylab="", xlab="", col="red")
abline(fit, col="red", lwd=2)

#regression line from the j-th sample of 10 cars
nsamp=10
#smp.j=sample(n,nsamp)
smp.j = c(8,  28,  39, 117,  63, 133, 107, 78, 55, 2)
dat.j = dat[smp.j,]
fit.j=lm(city~wt, data=dat.j)
points(dat.j$wt, dat.j$city, pch=17)
...
#j sample of 10 cars
nsim = 5000
predx=seq(2.5,5.5,.5); nx = length(predx)
j.results=matrix(NA,nsim,3)
j.ci95 = matrix(NA,nsim,3*nx)
j.ci75 = matrix(NA,nsim,3*nx)
for(i in 1:nsim){
  tmp.fit=lm(city~wt, data=dat, subset=sample(n, nsamp))
...
plot(dat$wt, dat$city, ylim=c(0,35), type="n", ylab="", xlab="")  
title("1000 bootstrapped regression lines\nfrom 10 car samples",cex.main=.75)
for(i in 1:1000){ lines(c(0,6), j.results[i,1]+j.results[i,2]*c(0,6)) }
legend("topright","(b)",bty="n")
abline(fit, col="red", lwd=2)

#confidence intervals
plot(dat$wt, dat$city, ylim=c(0,35), type="n", ylab="mpg", xlab="weight")
abline(fit, col="red", lwd=2)
points(dat.j$wt, dat.j$city, pch=17)
abline(fit.j)
