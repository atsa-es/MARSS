...
%~~~~~~~~~~~~~~~~~~~~~~~~~
These residuals should have (asymptotically) a $t$-distribution \citep[sec. 3]{KohnAnsley1989} and by looking at the residuals, we can identify potential outlier data points--or more accurately, we can identify data points that do not fit the model (Equation \ref{eq:random.walk.w.noise}).  The call \verb@MARSSresiduals()@ will compute these residuals for a marssMLE object (output by a MARSS call).   It returns the standardized residuals (also called auxiliary residuals) as a $n+m \times T$ matrix.  The first $n$ rows are the estimated $\vv_t$ standardized observation residuals and the next $m$ rows are the estimated $\ww_t$  standardized state residuals (discussed below). \index{functions!residuals}

<<Cs09_compute-resids, eval=TRUE, echo=TRUE>>=
resids.0 <- MARSSresiduals(kem.0)$std.residuals
resids.1 <- MARSSresiduals(kem.1)$std.residuals
resids.2 <- MARSSresiduals(kem.2)$std.residuals
@
...
kem.3 <- MARSS(dat, model = model, inits = kem.3$par, method = "BFGS", silent = TRUE)
summary(kem.3)
@


Now we will compute the model residuals as before:
<<compute-var-hat-vt2, echo=TRUE>>=
resids <- MARSSresiduals(kem.3)$std.residuals
@

The Nile data is not a good example since the variance for the slope is close to zero so the residual line is just a flat 0.
\begin{figure}[htp]
\begin{center}
<<plotslopetests-nile, echo=TRUE, fig=TRUE>>=
library(Hmisc)
par(mfrow = c(2, 1), mar = c(4, 3, 2, 1))
x <- seq(tsp(Nile)[1], tsp(Nile)[2], tsp(Nile)[3])
...
@
\end{center}
\caption{The standardized residuals for null model against one with level changes and slope changes for the Nile data. The slope plot is flat because the variance for the slope is almost zero.}
\label{fig:CS6.fig3}
\end{figure}


Let's run the same model on the \verb@treering@ dataset.
...
\begin{figure}[htp]
\begin{center}
<<plotslopetests-www, echo=TRUE, fig=TRUE>>=
library(Hmisc)
par(mfrow = c(2, 1), mar = c(4, 3, 2, 1))
x <- seq(tsp(WWWusage)[1], tsp(WWWusage)[2], tsp(WWWusage)[3])
plot(x, resids[2, ], ylab = "", xlab = "", type = "l", ylim = c(-4, 4), 
     xaxp = c(tsp(WWWusage)[1]-1, tsp(WWWusage)[2], 10))
minor.tick(nx = 10, ny = 0, tick.ratio = .3)
abline(h = c(1.97, -1.97), lty = 2)
title("test for level changes")

plot(x, resids[3, ], ylab = "", xlab = "", type = "l", ylim = c(-4, 4), 
     xaxp = c(tsp(WWWusage)[1]-1, tsp(WWWusage)[2], 10))
minor.tick(nx = 10, ny = 0, tick.ratio = .3)
abline(h = c(1.97, -1.97), lty = 2)
title("test for slope changes")
