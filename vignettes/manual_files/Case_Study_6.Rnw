...
kem.3 <- MARSS(dat, model = model, inits = kem.3, 
               method = "BFGS", silent = TRUE)
summary(kem.3)
@


The Nile data is not a good example since the variance for the slope is close to zero so the residual line is just a flat 0 (see value for \verb@Q.p@). Let's run the same model on the \verb@WWWusage@ dataset (Figure \ref{fig:CS6.fig3}).

\begin{figure}[htp]
\begin{center}
<<plotslopetests-nile, echo=FALSE, fig=TRUE>>=
plot(WWWusage)
@
\end{center}
