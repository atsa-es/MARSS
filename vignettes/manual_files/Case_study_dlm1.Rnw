...
%----------------------------------------------
\citet{ScheuerellWilliams2005} were interested in how well upwelling could be used to forecast expected survival of salmon. Let's look at how well our model does in that context. To do so, we need the predictive distributions for the regression parameters and observation.
