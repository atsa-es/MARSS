...
& tT & tt & tt1 & model & state & none & Cholesky & marginal & Block Cholesky\\ 
  \hline
\texttt{residuals(kfas_obj, type = "recursive")}&&&X&X&&X&&&X \\
...
\texttt{rstandard(kfas_fit$model, type = "recursive", standardization_type = "cholesky")}&&&X&X&&&X&&X \\
\texttt{rstandard(kfas_fit$model, type = "pearson", standardization_type = "marginal")}&X&&&X&&&&X& \\
\texttt{rstandard(kfas_fit$model, type = "pearson", standardization_type = "cholesky")}&X&&&X&&&X&&X \\
...
resid_marss <- residuals(fit_marss, type="tt1", standardization = "marginal")
resid_marss <- subset(resid_marss, name=="model")
df <- cbind(MARSS=resid_marss$.std.resids, KFAS = as.vector(resid_kfas))
head(df)
@
...
resid_marss <- residuals(fit_marss, type="tT", standardization = "Block.Cholesky")
resid_marss <- subset(resid_marss, name=="state")
df <- cbind(MARSS=resid_marss$.std.resids, KFAS = as.vector(resid_kfas))
head(df)
@
