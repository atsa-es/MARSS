...
<<>>=
resid_kfas <- predict(fit_kfas$model, interval = "confidence", se.fit = TRUE)
head(conf_kfas)
@
In MARSS, the same prediction is returned by \verb@fitted()@. By default \verb@fitted()@ returns a data frame, but the output can be changed to return matrices.
<<>>=
...
We can plot the confidence intervals and predictions as:
\begin{figure}[htp]
\begin{center}
...
\label{fig:KFAS.smooth.pred}
\end{figure}

With MARSS, there is a plot method (and ggplot2::autoplot method) for marssMLE objects which will make the smoothed model predictions with CIs and PIs.
\begin{figure}[htp]
\begin{center}
<<label=Cs31_marss-mult-fig-2, keep.source=TRUE, results=hide, fig=TRUE, echo=FALSE>>=
...
\label{fig:MARSS.smooth.pred}
\end{figure}
Alternatively you could used the fitted output.
\begin{figure}[htp]
\begin{center}
<<label=Cs31_marss-mult-fig-2, keep.source=TRUE, results=hide, fig=TRUE, echo=FALSE>>=
require(ggplot2)
...
\label{fig:MARSS.smooth.pred.manual}
\end{figure}
...
Get the confidence intervals on the estimate of the river flow (model fit conditioned on all the data).
<<>>=
...
\label{fig:MARSS.smooth.conf.NA}
\end{figure}

Compare model fitted values using all the data (smoothed) versus one-step-ahead estimates.
<<>>=
...
This example uses two series of average global temperature deviations for years 1880-1987 using two observation time series. This is a multivariate local level model with only one state process but two observation processes.
