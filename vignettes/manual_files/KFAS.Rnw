...
Both KFAS and MARSS return the smoothed and filtered (one-step ahead) model predictions via \verb@fitted()@. However, with KFAS this just returns the values. The \verb@KFAS::KFS()@ function will also return the filtered and smoothed model predictions in matrix form along with other filter and smoother output.
