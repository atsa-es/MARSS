...
Both KFAS and MARSS return the smoothed and filtered (one-step ahead) model predictions via \verb@fitted()@. However, for KFAS this just returns the smoothed values. The \verb@KFAS::KFS()@ function will  return the filtered and smoothed model predictions in matrix form along with other filter and smoother output.
...
The MARSS semantics are first letter x or y process, second letter time (usually t), and third letter the time conditioning. So \verb@ytT@ means the estimate of the $\yy$ process conditioned on all the data while \verb@ytt1@ means the estimate of the $\yy$ process conditioned on the data 1 to $t-1$. 
...
<<Cs708_globaltemp, include=FALSE, results=hide>>=
...
Prepare the model for KFAS.
<<Cs802_ARMA>>=
model_arima <- SSModel(y ~ SSMarima(ar = c(0, 0), ma = c(0, 0), Q = 1), H = 0)
@
A custom likelihood function is required for fitting this model with KFAS.
<<Cs803_ARMA>>=
...
# AR coefficients:
model_arima_fit$T[2:3, 2, 1]
# MA coefficients:
model_arima_fit$R[3:4]
# sigma2:
model_arima_fit$Q[1]
# intercept
KFS(model_arima_fit)
