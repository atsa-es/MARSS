...
@
<<Cs_00_required_libraries, echo=FALSE>>=
library(MARSS)
@

\chapter{Lag-p models with MARSS}
\label{chap:ARMA}
\chaptermark{Models with lag-p}
...
\item[ARSS(p)] The same as AR(p) but with a observation model and observation error.  The observations may be multivariate ($\yy$ can be multivariate) but the $\xx$ term is univariate.
...
Also keep in mind when reading this chapter that the MARSS package algorithms are fundamentally different than the default ARMA algorithms in that stationarity is not assumed in the MARSS algorithms.  The classic \citet{Gardneretal1980} approach of treating the initial condition ($x_0$) as coming from the stationary distribution is also not possible with the MARSS algorithms, since the stationary distribution depends on the parameters that one is estimating and imposes a non-linear constraint.  The default behavior of MARSS is to treat the initial condition as an estimated parameter, not an option in traditional ARMA algorithms.  You could fix the initial conditions to be the first $p$ values.  However then one has the conditional likelihood (conditioned on the first $p$ values).  For a model with Gaussian errors, you the conditional maximum-likelihood is the same as the conditional sum of squares, which for \verb@arima()@ you can specify using \verb@method="CSS"@.
...
By the way, there are much better and faster functions in \R for fitting univariate AR models (no observation error).  The MARSS package is really for fitting to multivariate data with observation error not AR-p models.  For example, here is how you would fit the AR(2) model using the base \verb@arima()@ function:
<<Cs_05_arima>>=
arima(gappy.data,order=c(2,0,0),include.mean=FALSE)
@
The estimates will be different because \verb@arima@ sets $\xx_1^0$ as coming from the stationary distribution.  That is a non-linear constraint that MARSS cannot handle.

The assumption that $\xx_1^0$ comes from the stationary distribution is fine if the initial $\xx$ indeed comes from the stationary distribution, but if the initial $\xx$ is well outside the stationary distribution the the estimates will be incorrect.
<<Cs_05b_non-stationary>>=
TT=50
true.2=c(r=0,b1=-1.5,b2=-0.75,q=1)
