...
\begin{equation}
\var_{XY_t|Y^{(1)}}[f(\YY_t,\XX_t)] = \var_{Y^{(1)}}[\E_{XY_t|Y^{(1)}}[f(\YY_t,\XX_t)|\YY^{(1)}]] + \E_{Y^{(1)}}[\var_{XY_t|Y^{(1)}}[f(\YY_t,\XX_t)|\YY^{(1)}]]
\end{equation}
where $f(\YY_t,\XX_t)$ is some function of $\XX_t$ and $\YY_t$ and $\YY^{(1)}$ is the observed data from $t=1$ to $T$. In this report, all expectations and variances are either over the data $\YY^{(1)}$ or the joint unconditional distribution of $\XX_t$ and $\YY_t$ ($(\YY_t,\XX_t)$) or the joint conditional distribution conditioned on the data ($(\YY_t,\XX_t)|\YY^{(1)}$). If the expectation or variance is over the data, $\YY^{(1)}$, then the subscript on the $\E$ or $\var$ is left on to clarify that $\YY^{(1)}$ is being integrated over. If the subscript on $\E$ or $\var$ is missing, the expectation or variance is over the joint distributio\footnote{If only one of the random variables appears, $\E_{AB}[A]=\E_{A}[A]$, but if we think about all the expectations and variances being over the joint distribution, we do not have to worry about operations like $\E[A|b]=\E[C|b]=\E[A+B]}. Whether it is conditional or unconditional can be seen by whether $|\YY^{(1)}$ or $|\yy^{(1)}$ appears.
...
\end{equation}
Notice that $\E[\WW_t|\YY^{(1)}] = $\hat{\WW}_t$.
\begin{equation}
\E[\WW_t|\YY^{(1)}] = \E[(\XX_t - \BB_t \XX_{t-1} - \uu_t)|\YY^{(1)}] = \E[\XX_t|\YY^{(1)}] - \E[\BB_t \XX_{t-1}||\YY^{(1)}] - \E[\uu_t|\YY^{(1)}] = \hatxt - \BB_t \hatxtm - \uu_t = \hat{\WW}_t
\end{equation}
Using  $\hat{\WW}_t=\E[\WW_t|\yy^{(1)}]$ in Equation \ref{eq:Wlawoftotvar}, we can write:
\begin{equation}
...
\end{split}
\end{equation}
Again this is conditional multivariate normal variance does not depend on the actual value, $\yy^{(1)}$ that we are conditioning on.  It depends only on the parameters values, $\QQ$, $\BB$, $\RR$, etc., and is the same for all value of $\yy^{(1)}$. So $\E_{Y^{(1)}}[\var[\WW_t|\YY^{(1)}]] = \var[\WW_t|\yy^{(1)}]$, using any value of $\yy^{(1)}$.
...
For a specific value of $\YY^{(1)}=\yy^{(1)}$, the covariance in the second term on the right is \cov[\hat{\VV}_t, \hat{\WW}_t|\yy^{(1)}]. Conditioned on a specific value of $\YY^{(1)}$, $\hat{\WW}_t$ is a fixed value because it is 
\begin{equation}
\hatxt - \BB_t\hatxtm - \uu_t = \E[\XX_t|\yy^{(1)}] - \BB_t \E[\XX_{t-1}|\yy^{(1)}] - \uu_t
\end{equation}
Conditioned on $\yy^{(1)}$, $\hatxt$ and $\hatxtm$ are fixed values and $\uu_t$ is fixed because it is a parameter. Thus conditioned on any $\yy^{(1)}$, $\hat{\WW}_t$ is a fixed value.  $\hat{\VV}_t$ is not a fixed value because it has $\YY_t$ and that is not fixed conditioned on $\yy^{(1)}$ because the $\yy^{(2)}$ in $\YY$ are still random variables.  So $\cov[\hat{\VV}_t, \hat{\WW}_t|\yy^{(1)}]$ is the covariance between a random variable and a fixed variable. That covariance is 0. This applies for all $\yy\yy^{(1)}$, so $\cov[\hat{\VV}_t, \hat{\WW}_t|\YY^{(1)}] = 0$ (notice $\YY^{(1)}$ in conditional not $\yy^{(1)}$ now).

With $\cov[\hat{\VV}_t, \hat{\WW}_t|\YY^{(1)}] = 0$, Equation \ref{eqn:covhatVtWt1} reduces to
\begin{equation}\label{eqn:covhatVtWt3}
\cov[\hat{\VV}_t,\hat{\WW}_t] = \cov_{Y^{(1)}}[\E[\hat{\VV}_t|\YY^{(1)}],\E[\hat{\WW}_t|\YY^{(1)}]]
\end{equation}

$\E_{\hat{X}|\yy^{(1)}}[\E[\XX_t|\YY^{(1)}]|\yy^{(1)}]=\E_{X|\yy^{(1)}}[\XX_t|\yy^{(1)}]$ since conditioned on $\yy^{(1)}$, $\E[\XX_t|\YY_t]$ is fixed\footnote{This would not be the case if the inner and outer expectations were not conditioning on the same random variable.} at $\E[\XX_t|\yy_t]$ ($\hatxt$). Because the only random variables in $\hat{\WW}_t$ are $\hatxt$ and $\hatxtm$, this means  $\E[\hat{\WW}_t|\yy^{(1)}]=\E[\WW_t|\yy^{(1)}]$. $\E_{\hat{W}|y}[\hat{\WW}_t|\yy^{(1)}]=\E_{W|y}[\WW_t|\yy^{(1)}]$

Since $\E_{\hatxt|\yy^{(1)}}[\E[\XX_t|\YY_t]|\yy^{(1)}]=\E_{X|\yy^{(1)}}[\XX_t|\yy^{(1)}]$ (and same for $\XX_{t-1}), we know that $\E[\hat{\WW}_t|\yy^{(1)}]=\E[\WW_t|\yy^{(1)}]$ and $\E[\hat{\VV}_t|\yy^{(1)}]=\E[\VV_t|\yy^{(1)}]$. Since this applies for all $\yy^{(1)}$, In the same way, we can show that $\E[\hat{\VV}_t|\yy^{(1)}]=\E[\VV_t|\yy^{(1)}]$.
#####################

Thus Equation \ref{eqn:covhatVtWt3} can be written in terms of $\VV_t$ and $\WW_t$:
