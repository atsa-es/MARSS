...
\end{split}
\end{equation}
$\hatVt = \var[ \XX_t | \yy^{(1)} ]$ and is output by the Kalman smoother. $\hatUt=\var[\YY_t|\yy^{(1)}]$ and $\hatSt=\cov[\YY_t,\XX_t|\yy^{(1)}]$. The equations for these are given in \citet{Holmes2010} and can be computed from the output by the \verb@MARSShatyt@ function in the MARSS R package.
...
\var[\hat{\VV}_t] = \var_Y[\YY_t] - \ZZ_t\E[\XX_t|\yy^{(1,1:t-1)}]
\end{equation}
