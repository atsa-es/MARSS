...
############# NOT TRUE
We know that $\E[\hat{\WW}_t|\yy^{(1)}]=\hat{\ww}_t$ (that's the $\hat{\ww}_t$ definition) and we know that $\hat{\ww}_t = \E[\WW_t|\yy^{(1)}]$ (as shown above). Thus $\E[\hat{\WW}_t|\yy^{(1)}]=\E[\WW_t|\yy^{(1)}]$. This applies for all $\yy^{(1)}$ so $\E[\hat{\WW}_t|\YY^{(1)}]=\E[\WW_t|\YY^{(1)}]$.  In the same way, we can show that $\E[\hat{\VV}_t|\yy^{(1)}]=\E[\VV_t|\yy^{(1)}]$.
#####################
