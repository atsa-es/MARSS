...
The marginal standardized joint residuals for $\widehat{\varepsilon}_t$ (both model and state one-step ahead residuals) can be computed with the inverse of the square root of the diagonal of $\overline{\Sigma}_t$:
\begin{equation}
\dg(\overline{\Sigma}_t)^{-1/2} \widehat{\varepsilon}_t
\end{equation}
...
The unconditional model residuals, $\VV_t$ and $\VV_{t+1}$, are independent, but the conditional model residuals, $\cov[\checkVt,\checkVtp]$, are not independent since both depend on $\yy^{(1)}$ up to $t$.  
Using the law of total covariance, we can write
\begin{equation}\label{eq:covhatVtVtp}
\cov[\checkVt,\checkVtp] = 
\cov_{Y^{(1)}}[\E[\checkVt|\YY^{(1)}],\E[\checkVtp|\YY^{(1)}]] + \E_{Y^{(1)}}[\cov[\checkVt, \checkWt|\YY^{(1)}]] .
\end{equation}
