...
\end{equation}
where $\hatxtmt1$ is the $\E[\XX_{t-1}|\yy^{(1),t-1}]$ (expected value of $\XX_{t-1}$ conditioned on the data up to time $t-1$). However $\hatxttm = \E[\XX_t|\yy^{(1),t-1}] = \BB_t\hatxtmt1 + \uu_t$. Thus $\checkwt = 0$.
...
\begin{bmatrix}\yy_t - \ZZ_t\hatxttm-\aa_t\\ \hatxttm - \BB_t\hatxtmt1 - \uu_t \end{bmatrix}.
\end{equation}
where $\hatxttm$ and $\hatxtmt1$ are conditioned on the observed $\yy$ from $t=1$ to $t-1$.
...
\begin{bmatrix}\yy_t - \ZZ_t\hatxttm-\aa_t\\ \hatxtpt1 - \BB_{t+1}\hatxttm-\uu_{t+1} \end{bmatrix}
\end{equation}
