...
It should be noted that all the calculations discussed here are conditioned on the MARSS parameters: $\BB$, $\QQ$, $\UU$, $\RR$, $\ZZ$ and $\AA$. These are treated as known. This is different than standard discussions of residual distributions for linear models where the uncertainty in the model parameters enters into the calculations (enters into the calculation of the influence of $\yy$ on the expected (or fitted) value of $\yy$). In these calculations, $\yy$ does not affect the estimates of the parameters (which are fixed, perhaps or most likely at estimated values) but does affect the expected value of $\yy$ by affecting the estimate of $\xx$. 
...
The standardized residuals are computed by multiplying $\widehat{\varepsilon}_t$ by $(\widehat{\Sigma}_t)^{-1/2}$ (the inverse of the Cholesky decomposition of $\widehat{\Sigma}$ of the variance-covariance matrix for $\widehat{\varepsilon}_t$):
\begin{equation}\label{eq:std.resid}
\text{standardized residuals} \widehat{\varepsilon}_{t, std} = (\widehat{\Sigma}_t)^{-1/2}\widehat{\varepsilon}_t .
\end{equation}
These residuals are uncorrelated (across the $n$ residuals in $\widehat{\varepsilon}_t$).

It is also common to use the marginal standardized residuals, which would be $\widehat{\varepsilon}_t$ multiplied by the inverse of the square-root of the diagonal of $\widehat{\Sigma}_t$:
\begin{equation}
\text{i-th marginal standardized residual} \widehat{\varepsilon}(i)_{t, mar.std} = \frac{\widehat{\varepsilon}_{i,t}}{\sqrt{\widehat{\Sigma}(i,i)_t}}.
\end{equation}
These residuals may be correlated across the $n$ residuals in $\widehat{\varepsilon}_t$ (depending on the structure of the MARSS model) but a bit easier to interpret when there is correlation across the $n$ residuals.

See \citet{HarveyKoopman1992} and \citet{Harveyetal1998} for a discussion on how to use these residuals for outlier detection and structural break detection.
...
Define the innovations $\checkvt$ as\footnote{This is slightly different than the observed `innovations' as it includes both $\yy_t^{(1)}$ and $\yy_t^{(2)}$. Also $\yy_t$ here is not the actual data that you observe (the one data set that you have). It's the data that you could observe. $\yy_t$ is a sample from the random variable $\YY_t$.}:
...
Note $\var[\checkVt]$ is a standard output from Kalman filter functions and is used to compute the likelihood of the data (conditioned on a set of parameters). In the Kalman filter in the MARSS R package, it is output as \texttt{Sigma} from \texttt{MARSSkf()}.
...
\end{equation}
where $\KK_t$ is the Kalman gain\footnote{In the MARSS R package, this is output by \texttt{MARSSkfss()} in \texttt{Kt}}. $\KK_t = \hatVtt1 \ZZ_t^\top[\ZZ_t \hatVtt1 \ZZ_t^\top + \RR_t]^{-1}$.
...
The covariance of $\cov[\checkVt, \checkWt]$ is
\begin{equation}
\cov[\checkVt, \checkWt] = \cov[\checkVt, \KK_t \checkVt] =
[\RR_t + \ZZ_t \hatVtt1 \ZZ_t^\top] \KK_t^\top
\end{equation}
...
\begin{bmatrix}\yy_t - \ZZ_t\hatxttm-\aa_t\\ \hatxtt - \BB_t\hatxtmt1 - \uu_t \end{bmatrix}.
\end{equation}
The expected value of $\overline{\mathcal{E}}_t$ over all possible $\yy$ up to time $t$ is 0 and the variance of $\overline{\mathcal{E}}_t$  is
\begin{equation}\label{eq:jointcondresid1innovs}
\overline{\Sigma}_t = \begin{bmatrix}[c|c]
 \var[\checkVt]&
 \cov[\checkVt, \checkWt] \\
 ...
 \begin{bmatrix}[c|c]
 \RR_t + \ZZ_t \hatVtt1 \ZZ_t^\top& [\RR_t + \ZZ_t \hatVtt1 \ZZ_t^\top] \KK_t^\top \\
 \rule[.5ex]{25ex}{0.25pt} & \rule[.5ex]{25ex}{0.25pt} \\
\KK_t [\RR_t + \ZZ_t \hatVtt1 \ZZ_t^\top] & \KK_t [\RR_t + \ZZ_t \hatVtt1 \ZZ_t^\top] \KK_t^\top \end{bmatrix} .
\end{equation}

The standardized residuals for $\overline{\varepsilon}_t$ can be computed with $\overline{\Sigma}_t$ in the same way that they were computed for $\widehat{\varepsilon}_t$ with $\widehat{\Sigma}_t$ (Equation \ref{eq:std.resid}).
