...
\end{equation}
Conditioned on $\yy^{(1)}$, $\hatxt$ and $\hatxtm$ are fixed values and $\uu_t$ is fixed because it is a parameter. $\hat{\VV}_t$ is not a fixed value because it has $\YY_t^{(2)}$ and that is a random variable.  So $\cov[\hat{\VV}_t, \hat{\WW}_t|\yy^{(1)}]$ is the covariance between a random variable and a fixed variable and that covariance is 0. So \E_{Y^{(1)}}[$\cov[\hat{\VV}_t, \hat{\WW}_t|\YY^{(1)}]] = 0$.
