...
If you already work with models in the form of Equation \ref{eqn:marss}, you can immediately fit your model with the MARSS package.  Install the MARSS package and then type \texttt{library(MARSS)} at the command line to load the package.  Look at the Quick Start Guide and then skim through Chapter \ref{chap:Examples}.  The Chapter \ref{chap:modelspec} in the appendix also has many examples of how to specify different forms for your parameter matrices. 
...
\end{Sinput}
\end{Schunk}
When you call MARSS, \verb@dat[c("Temp1","Temp2"),]@ is the data. \verb@dat[c("Year","Month","Salinity"),]@ are your covariates.  
