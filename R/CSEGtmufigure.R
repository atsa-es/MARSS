...
for(j in 1:ngrid) {
	T=Tvals[j];
	safe.limits[j]=uniroot( function(x) {TMUpper(T,x)-0.05}, 	lower=minval,upper=maxval)$root; 
	dead.limits[j]=uniroot( function(x) {TMLower(T,x)-0.95}, 	lower=minval,upper=maxval)$root;
}
...
abline(h=-2.2); abline(h=0); abline(v=100);  abline(v=1);
offset = -0.05
abline(h=-.3); text(5,-.3+offset,"50%")
