toLatex.marssMODEL = function(object, ..., file=NULL, digits=2, greek=TRUE, orientation="landscape", math.sty="amsmath", output=c("pdf","tex"),replace=TRUE,simplify=TRUE){
  ...
  m=dim(fixed$x0)[1]; n=dim(x$data)[1]; TT=dim(x$data)[2]
  ...
  for(i in 1:length(eqns)){
    ...
    #############################################################
      ...
      #########################################
      if(!time.expand) time.sub=eqn.name.time
      else time.sub=paste("{",as.character(t),"}",sep="")
      ...
      ##############################################################
      #start with the eqn name (left of ~) 
      if(!time.expand) time.sub=err.name.time
      else time.sub=paste("{",as.character(t),"}",sep="")
      ...
      tmp=strsplit(eqn.stoc,"[(]")[[1]][1]; tmp=str_trim(strsplit(tmp,"~")[[1]][2])
      #wrap in pmatrix
  ...
  for(el in strsplit(eqn,"[+]")[[1]]){
    ...
    for(el2 in strsplit(el,"[*]")[[1]]){
      ...
      if(str_detect(el2,"_")){
        ...
        else el2.time=paste("{",as.character(t),"}",sep="")
      ...
      if(is.numeric(el2)){ mat.tex=as.character(el2)
      }else{
...
parameters.time.varying = function(eqn, eqn.special, x){
  ...
  fixed=x$fixed
  free=x$free
  if(!is.null(attr(x,"model.dims"))){ model.dims=attr(x,"model.dims")
