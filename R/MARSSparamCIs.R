...
#   If method='hessian', uses the Harvey 1989 algorithm 
#     which gives an analytical solution to the hessian.  Numerical estimate, possible too.
#   If method='numerical', it uses nlme::fdHess in MARSShessian.chol(), called by MARSSFisherI(..., method="numerical")
#     the Hessian for the variance will be for the chol transformed var-cov matrices,
#     so a back transformation is needed.  If the matrix is diagonal, then there is 
#     an analytical solution using the 1-4 moments.  If the matrix is unconstrained, #     then the s.e. and CIs are obtained by simulating from the Hessian.
####################################################################################
MARSSparamCIs = function(MLEobj, method="hessian", alpha=0.05, nboot=1000, silent=TRUE) {
  ...
  if(!(method %in% c("hessian","parametric","innovations","hessian-numerical", "hessian-harvey1989"))) stop("Stopped in MARSSparamCIs(). Current methods are hessian, parametric, innovations, hessian-numerical and hessian-harvey1989.\n", call.=FALSE)
  ...
  if(method=="hessian"){ #choose method from MLEobj method
    #need package globals
    kem.methods=get("kem.methods", envir=pkg_globals)
    optim.methods=get("optim.methods", envir=pkg_globals) 
    
    if(MLEobj$method%in%kem.methods) method="hessian-harvey1989"
    if(MLEobj$method%in%optim.methods) method="hessian-numerical"
  }
  if(method=="hessian-numerical")  {
    #Only for fixed, diagonal or block-diagonal unconstrained R, Q, or V0
    RQV0type=ok.form=c(Q=NA, R=NA, V0=NA)
    tmp=coef(MLEobj, type="matrix")
    for(elem in c("R","Q","V0")){
      ok.form[elem] = is.validvarcov(tmp[[elem]], method="BFGS")$ok
      if(!ok.form[elem]){ RQV0type[elem]="illegal"; next }
      if(length(MLEobj$par[[elem]])==0){ RQV0type[elem]="fixed"; next }  
      if(is.diagonal(tmp[[elem]])){ RQV0type[elem]="diagonal"; next } 
      #if legal, then either fixed, diagonal or block unconstrained
      RQV0type[elem]="unconstrained"
    }
    if(any(!ok.form)) stop("MARSSparamCIs: To use method=hessian-numerical, R, Q, and V0 must be either fixed, diagonal or block diagonal unconstrained.")
    #Run nlme::fdHess to get hessian
    #MLEobj that is returned has var-cov matrices chol transformed
    MLEobj.hessian = MARSSFisherI(MLEobj, method="numerical")
    ...
    if(inherits(hessInv, "try-error") ) {
      ...
      vector.par.se[!is.neg] = try(sqrt(diag(hessInv)[!is.neg]), silent=TRUE)
      #invert the Hessian; wrap in a try() in case it fails
    ...
    if(any(RQV0type=="diagonal")){ 
      #back transform the standard error to be on the original (not chol) scale
      for(elem in c("Q","R","V0")[RQV0type=="diagonal"]){
        loc=grepl(elem, names(vector.par.upCI)) #where the parameter is in the vec
        tmp=MARSShessian.chol.backtrans(MLEobj.hessian, vector.par.upCI)
    ...
    if(any(RQV0type=="unconstrained")){ #need to back transform with simulation
      for(elem in c("Q","R","V0")[RQV0type=="unconstrained"]){
        loc=grepl(elem, names(vector.par.upCI)) #where the parameter is in the vec
        Mu=MLEobj.hessian$parMean
        Sigma=MLEobj.hessian$parSigma
        a=mvrnorm(nboot,Mu,Sigma)
        elem.boot=matrix(NA,nboot,sum(loc))
        for(i in 1:nboot){
          tmp.MLE=MARSSvectorizeparam(MLEobj.hessian,a[i,])
  ...
  if(method=="hessian-harvey1989")  {
    ...
    MLEobj.hessian = MARSSFisherI(MLEobj, method="Harvey1989")
    #deal with NAs in the Hessian
