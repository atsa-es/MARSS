###############################################################################################################################################
#  Internal function to return the predicted y and x
##############################################################################################################################################
MARSSpredict <- function(object,
  ...
  if (is.null(MLEobj[["par"]])) {
    stop("MARSSpredict: The marssMLE object does not have the par element.  Most likely the model has not been fit.", call. = FALSE)
  }
  if (interval != "none" && (!is.numeric(level) || length(level) != 1 || level > 1 || level < 0))
    stop("MARSSpredict: level must be a single number between 0 and 1.", call. = FALSE)
  alpha <- 1-level
