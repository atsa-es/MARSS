...
#     for the cholesky transformation of the var-cov matrices.
#     The transformation ensure they stay positive definite
#
...
MARSShessian.chol = function(MLEobj) {
  ...
  ##
  # This function is passed into the optim() call
  # NOTE: the MLEobj in the call is a MLEobj that has the variances chol transformed
  # this function is dealing with that transformation
  kfNLL = function(x, MLEobj=NULL){  #NULL assignment needed for optim call syntax
    ...
    #transform variance parameters back to the untransformed state
    par.untransformed=MARSShessian.backtrans(MLEobj, x)
    #A holder
    MLEobj.orig=MLEobj
    #MARSSoptim() added on $fixed.original and $free.original so these would be available here
    MLEobj.orig$marss$fixed = MLEobj$fixed.original
    MLEobj.orig$marss$free = MLEobj$free.original
    #reconstruction the original, untransformed, MLE obj
    MLEobj.orig=MARSSvectorizeparam(MLEobj.orig, par.untransformed)
    
    #Now we can use MARSSkf() to compute the neg LL needed in the optim function
  ...
  #This code is used to set things up to use MARSSvectorizeparam
  #to just select inits for the estimated parameters
  tmp.MLEobj = MLEobj
  #This is needed inside the kfNLL function above
  ...
  for(elem in c("Q","R","V0")){ #need the chol for these
    if(!is.fixed(free[[elem]])){
      ...
      for(t in 1:max(TT.f,TT.d)){
        ...
        if(!tmp$ok) stop("Stopped in MARSShessian(): The variance matrix must be diagonal for the Hessian computation.")
        #I think you can have time-varying but I need to figure how to compute fixed and free using the t=1 par; 
        
        #what's happening here is I am chol-transforming the var-cov matrix.  
        #The matrix can have fixed and shared elements.
...
MARSShessian.chol.backtrans = function(MLEobj.hessian, par.hessian){
  ...
  #Goal is to return the untransformed parameter values
