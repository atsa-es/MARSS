...
cbind(P_mu=f_kfas$P_mu[1:n], fitted=var.Eytt1_fit[1:n], 
...
###################################################
conf_kfas <- predict(fit_kfas$model, interval = "confidence", 
                     se.fit = TRUE)
head(conf_kfas)
...
inits <- MARSS(t(GlobalTemp), model=mod.list, control=list(maxit=20), 
               silent=TRUE)
marss_temp_stoch_bfgs <- MARSS(t(GlobalTemp), model=mod.list, 
...
rownames(vals) <- c("KFAS default", "MARSS em default", 
...
df <- data.frame(t=as.vector(time(coef(out_temp))),
                 KFAS=coef(out_temp),
...
resid_kfas <- rstandard(kfs, type = "state", 
                        standardization_type = "cholesky")
resid_marss <- residuals(marss_temp_stoch_bfgs, type="tT", 
                         standardization = "Block.Cholesky")
resid_marss <- subset(resid_marss, name=="state")
resid_test <- residuals(marss_test, type="tT", 
                         standardization = "Block.Cholesky")
resid_test <- subset(resid_test, name=="state")

df <- data.frame(MARSS=resid_marss$.std.resids, 
            KFAS = as.vector(resid_kfas),
...
ggplot(subset(df, model%in%c("diff.est","diff.id")), aes(x=t, y=value, col=model)) + 
  geom_line(na.rm=TRUE) + facet_wrap(~name) + xlab("") +
  ggtitle("Difference in residuals KFAS vs MARSS") 


###################################################
### code chunk number 66: Cs708_globaltemp
###################################################
...
marss_test <- MARSS(t(GlobalTemp), model=mod.list)
kfas_test_mod <- MARSSkfas(marss_test, return.kfas.model=TRUE, 
                              return.lag.one=FALSE)$kfas.model
...
###################################################
### code chunk number 67: Cs801_ARMA
###################################################
set.seed(1)
...
###################################################
### code chunk number 68: Cs802_ARMA
###################################################
model_arima <- SSModel(y ~ SSMarima(ar = c(0, 0), ma = c(0, 0), Q = 1), H = 0)


###################################################
### code chunk number 69: Cs803_ARMA
###################################################
...
fit_arima <- optim(par = c(rep(0, 4), log(1)), fn = likfn, method = "BFGS",
  model = model_arima)
model_arima_fit <- likfn(fit_arima$par, model_arima, FALSE)
